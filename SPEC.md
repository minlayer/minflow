...
| `model` | Optional per-node model override. Prefer a **portable tier**, `small` / `medium` / `large`, which each backend translates; a tier is relative to whichever ladder the deployment resolves, since a client may expose several providers' ladders at once. A provider's own name is still accepted and still leaks a platform fact into the IR, which is a known defect the type has not yet closed (D28, L23). A value the backend does not recognise is a compile error |
...
- Created by the entrypoint hook, destroyed at the terminal node, and replaced there by a **record**: the run's outputs, its outcome, and the graph hash it ran against, written to a separate `records/` directory. A record is not state and is not a run. Nothing resumes one, no scan of live runs sees one, and the entry command's `--from <node>` seeds a fresh run from one at a chosen node. It exists because editing a workflow means running it, changing one prompt, and running it again, and re-running twenty steps to reach the twenty-first is the whole cost of that loop. Records are capped at the most recent 20.
...
| **L23** | `model` and `tools` are typed as free strings in a provider-agnostic IR, so a provider's own model name is still legal there and a tool allowlist would carry a client's own tool names. A second backend cannot read such a graph without understanding the first provider's product line. | All | Partly contained in 0.2.1 (D28): portable tiers `small` / `medium` / `large` are accepted and translated per backend, and an unrecognised model is now a compile error naming the node rather than frontmatter naming a model that does not exist. Not fixed: the type is still `string`, so provider names remain legal. Closing that is breaking. The ladder itself, and any exact pin, belong in `EmitOptions` beside every other platform fact. `effort` is a second axis the IR still cannot express at all |
