...
/**
 * Deterministic JSON, with object keys recursively sorted.
 *
 * `JSON.stringify` preserves insertion order, so two structurally identical
 * specs written in a different order would hash to different keys and the host
 * would run the same observation twice. Sorting is by UTF-16 code unit, never
 * `localeCompare`, which is locale-dependent and would make the key impure
 * across machines.
 *
